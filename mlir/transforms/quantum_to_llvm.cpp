 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    for (int i = 0; i < qbits.size(); i++) {
      ...
      // should be Qubit* __quantum__rt__array_get_element_ptr_1d(Array*,
      // QubitIdx)
      auto array_qbit_type = LLVM::LLVMType::getInt64Ty(context).getPointerTo();
    ...
    // auto bitcast_type =
    // LLVM::LLVMType::getInt64Ty(context).getPointerTo().getPointerTo(); for
    // (auto qbit_value : qbit_values) {
    //   auto result = rewriter.create<LLVM::BitcastOp>(loc, bitcast_type,
    //   qbit_value); auto tmp = rewriter.create<LLVM::LoadOp>(loc,
    //   LLVM::LLVMType::getInt64Ty(context).getPointerTo(), result);
