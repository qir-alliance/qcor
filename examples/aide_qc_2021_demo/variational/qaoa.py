...
@qjit
def qaoa_circ(q: qreg, cost_ham: PauliOperator, steps: int, gamma: List[float], beta: List[float]):
...
def qaoa_translate(self, q: qreg, x: List[float]):
    ...
    ret_dict["q"] = q
    ret_dict["cost_ham"] = H
    ret_dict["steps"] = n_steps
    ret_dict["gamma"] = x[:n_steps]
